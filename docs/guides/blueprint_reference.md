...
Unreal Robotics Lab 中的每个 MuJoCo 组件都是普通的虚幻引擎组件。铰链是参与者（Actor），您可以像操作其他参与者或组件一样与它们交互：获取引用、调用函数、绑定事件。除非需要，否则无需直接操作 MuJoCo。
...
Mujoco 铰链参与者（`AMjArticulation`）本质上就是一个参与者。获取引用的方式与通常相同：

**蓝图：** 获取所有类的参与者、从命中结果进行类型转换，或存储引用变量。
...
从管理器中，您可以按名称查找铰链，或获取所有铰链：

**蓝图：** **获取管理器** → **获取铰链** (名称) / **获取所有铰链**
...
一旦你创建了一个铰链，它的 MuJoCo 组件就只是子组件。你可以按名称或数组访问它们：

**蓝图：** **获取执行器** (名称), **获取铰链** (名称), **获取传感器** (名称) — 或者对于数组 **获取执行器**, **获取铰链**, **获取传感器**, **获取刚体**。你也可以直接从蓝图中的组件树拖拽组件。
...
| **Reset Simulation** | 重置模拟：零位置、速度、时间 |
...
在 Mujoco 铰链参与者（`AMjArticulation`）中:
...
在关卡中使用 Mujoco 回放管理器`AMjReplayManager`
...
关于关键帧控制器、预设和 FMjKeyframePose 结构体的完整详细信息，请参阅 [控制器框架](controller_framework.md) 。

---

## MjKeyframeCameraActor

Mujoco 关键帧相机参与者 （`AMjKeyframeCameraActor`）是一款影视级相机，它能够平滑地通过一系列路径点（`FMjCameraWaypoint`：位置、旋转、时间）进行插值。它使用 `UCineCameraComponent`，并在编辑器中显示路径的样条曲线预览。
...
Mujoco 脉冲启动器 （`AMjImpulseLauncher`）会对 Mujoco 刚体（`MjBody`）施加基于速度的脉冲。有两种模式：

- **直接**模式 — 沿参与者的前向向量（或 `DirectionOverride`）启动。 
- **目标**模式 — 将启动目标（`LaunchTarget`）设置为某个参与者，它会计算指向该动作者的弹道弧线。`ArcHeight` 控制抛射高度。
