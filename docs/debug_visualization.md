...
由于叠加层会替换真实 `UStaticMeshComponents` 上的真实材质，因此任何渲染关卡的工具（例如 `USceneCaptureComponent2D`、`UMjCamera` 和截图工具）都会捕获叠加层，而不是底层场景。这对于合成数据管线来说是理想的（语义分割模式可以提供免费的真实掩码），但如果您需要 RGB 观测流保持照片级真实感，则这是不希望出现的。

目前可用的解决方法：

- 在捕获 RGB 帧之前，关闭叠加层（按下 `6` 键直至关闭）。
- 如果您正在生成训练数据，脚本会自动调整捕获窗口。
